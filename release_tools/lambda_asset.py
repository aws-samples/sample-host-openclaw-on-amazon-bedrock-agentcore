...
# The immutable capability catalog and its exact schema set ship inside the
# trusted asset alongside the Lambda source. They live under specs/capabilities
# in the repository and are packaged into capabilities/artifacts/ in the payload.
...
def _capability_source_inputs(root: Path) -> list[tuple[Path, str]]:
    """Bind the repository capability catalog and exact schema set as source.

    The catalog and schemas live under ``specs/capabilities`` beside the lambda
    source root and are packaged into ``capabilities/artifacts/`` in the payload,
    so a drifted or partial capability artifact set fails the source inventory.
    """

    capability_source = root.parent / "specs" / "capabilities"
    catalog = capability_source / "catalog-v1.json"
    schemas = sorted((capability_source / "schemas").glob("*.json"))
    if len(schemas) != _REQUIRED_CAPABILITY_SCHEMA_COUNT:
        raise ContractError(
            "trusted Lambda capability schema inventory is not exact"
        )
    inputs = [(catalog, f"{_CAPABILITY_ARTIFACT_PREFIX}catalog-v1.json")]
    inputs.extend(
        (path, f"{_CAPABILITY_SCHEMA_PREFIX}{path.name}") for path in schemas
    )
    return inputs
