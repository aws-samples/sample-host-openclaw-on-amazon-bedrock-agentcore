...
- **Tools & Skills**: Built-in tool groups (full profile) + 5 ClawHub skills + 3 custom skills (S3 user files, EventBridge cron, cost-analyzer) + 2 built-in shim tools (web_fetch, web_search)
- **Cost Analysis**: Dedicated Claude Agent SDK agent (`query()` + `createSdkMcpServer()`) with MCP servers — queries AWS Cost Explorer, CloudWatch Bedrock logs, and DynamoDB token usage for cross-referenced cost reports
...
openclaw-on-agentcore/
  ...
  bridge/
    ...
    skills/
      ...
      cost-analyzer/              # Cost analysis agent (Claude Code CLI + MCP servers)
        SKILL.md                  # OpenClaw skill manifest
        run.js                    # Agent runner (spawns claude --print with MCP config)
        token-usage-server.js     # Node.js MCP server for DynamoDB token usage queries
        system-prompt.md          # Agent system prompt (moved to /app/cost-analyzer-deps/ in container)
        aws-cost-server/server.py # Python MCP server for Cost Explorer + CloudWatch logs
  lambda/
...
### Cost Analyzer Agent
- **Claude Agent SDK**: Uses `query()` from `@anthropic-ai/claude-agent-sdk` with `CLAUDE_CODE_USE_BEDROCK=1` for autonomous multi-step reasoning via Bedrock
- **In-process MCP**: Token-usage tools built with `createSdkMcpServer()` + `tool()` — runs DynamoDB queries in-process (no subprocess)
- **Stdio MCP**: Python aws-cost-server (Cost Explorer + CloudWatch) spawned as stdio subprocess
- **Flat skill directory**: Only SKILL.md + code files in `/skills/cost-analyzer/`; supporting files (system-prompt.md, aws-cost-server/) live in `/app/cost-analyzer-deps/` to avoid OpenClaw skill scanner interference
- **IAM**: Needs `ce:GetCostAndUsage`, `ce:GetCostForecast`, `logs:FilterLogEvents`, `dynamodb:Query` on token usage table
