    ...
    context.update(
        {
            ...
            # The capability catalog is compiled from and pinned to the same
            # exact reviewed release commit that the runtime image is built at,
            # so the release synth binds the capability stack without a separate
            # mutable deploy flag.
            "capability_release_commit": runtime.source_commit,
        }
    )
    return context
